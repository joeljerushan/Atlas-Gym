...
//set structured data to Members 
struct Members{
...
//set member's pocket ID
int member_pocket_id, found=0; 

void Register(){ 
	//Register()function for registering members
	
	//when user enters to register function clear screen 
	system("cls"); 
	...
	cout<<"\n\n\tPlease Press ENTER key for Main Menu";
	 ...
	 //increment member's pocket ID by adding 1 dynamically
	member_pocket_id++;
	
	getchar();
	getchar();
	
	//when user enters clear screen 
	system("cls"); 
}
bool Login(){ 
	//Login() check user is logged or not using bool function
	
	//set login info to string
	string login_info[2]; 
	
	//set integer for login_info to check line number 
	int i=0; 
	
	//ifstream opens a file called log.txt and set it to log_info_file
	ifstream log_info_file("log.txt"); 
	
	if(log_info_file.is_open()){
		//check for lines on log.txt file
		while(getline(log_info_file,login_info[i++])); 
...
int main(){
	...
	if(Login()) {
		...
		//add start point id for check_menu_exit
		id=0; 
		...
		do
		{
			...
			//if user pressed deadly answer show error message 
			if (cin.fail())
			{
				//error message set to a function so called here 
				Error(); 
			}
		}
		//exit when 6 pressed 
